...
int main(int argc, const char* argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        if (dataBuffer.size() > dataBufferSize) dataBuffer.pop_front(); //ring buffer. Deque as it grows in both directions.
        ...
        // Task MP.7 Counting keypoints
         ...
         if (keypoints.size() > 0) {
            auto kp = keypoints[0];
             cout << kp.size << endl; //neighbourhood size
         }
        //// EOF STUDENT ASSIGNMENT
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            cout << descriptorType << ",";
            cout << matches.size() << ",";
            cout << 1000 * det_t / 1.0 << ","; //detector time
            cout << 1000 * desc_t / 1.0 << ","; //descriptor time
            cout << (1000 * det_t / 1.0) + (1000 * desc_t / 1.0) << endl; //total
            //// EOF STUDENT ASSIGNMENT
